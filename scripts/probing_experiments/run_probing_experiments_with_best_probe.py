...
def _print_results(results):
    ...
    print("\n" + "-" * 120)
    print(f"{'Experiment':<55} {'AUC':<16} {'PR-AUC':<16} {'F1':<16}")
    print("-" * 120)
    ...
    for name, result in sorted_results:
        ...
        pr_auc_std = result.get("pr_auc_std", 0)
        f1 = result.get("f1", 0)
        f1_std = result.get("f1_std", 0)
        print(
            f"{name:<55} "
            f"{auc:.3f}±{auc_std:.3f}     "
            f"{pr_auc:.3f}±{pr_auc_std:.3f}     "
            f"{f1:.3f}±{f1_std:.3f}"
        )
    print("-" * 120)
    print(f"\nTotal experiments: {len(results)}")
    if results:
        best_name, best_result = max(results.items(), key=lambda x: x[1].get("auc", 0))
        print(
            f"Best by AUC: {best_name} | "
            f"AUC={best_result.get('auc', 0):.3f}±{best_result.get('auc_std', 0):.3f}, "
            f"F1={best_result.get('f1', 0):.3f}±{best_result.get('f1_std', 0):.3f}"
        )
...
def run_with_query(data_path, output_dir, experiment_name, cv_folds):
    ...
    combined_path = output_path / f"probing_results_{experiment_name}_with_query_combined.json"
    serializable = {
        k: {key: _make_serializable(val) for key, val in v.items()}
...
def _get_best_result(results):
    """Return best experiment result by mean CV AUC."""
    if not results:
        return None, None
    best_name, best_result = max(
        results.items(), key=lambda x: x[1].get("auc", float("-inf"))
    )
    return best_name, best_result


def _fit_probe_full_data(X, y, classifier_type, config):
    """Fit the selected best probe on the full dataset."""
    if classifier_type in ["LinearProbeTorch", "MLPProbeTorch", "MLPSCLProbeTorch"]:
        X_train_split, X_val_split, y_train_split, y_val_split = train_test_split(
            X,
            y,
...
def _save_probe_artifacts(save_obj, save_prefix):
    """
    Save the fitted best probe.

    Always tries to save:
    - full probe object as *_model.pkl
    - scaler as *_scaler.pkl when present
    - torch state_dict as *_weights.pt when available
    """
    ...
    try:
        ...
        print(f"Warning: failed to save torch weights: {e}")
...
def save_best_probe(results, data, output_path, experiment_name, block_name):
    """Select best experiment by mean CV AUC, refit on full data, and save artifacts."""
    ...
    X = build_X(
        data=data,
        xrag_combo=xrag_features,
        query_combo=query_features,
    )
    y = data["labels"].numpy()
    ...
    print(
        f"Mean CV AUC: {best_result['auc']:.3f} ± {best_result.get('auc_std', 0.0):.3f}"
    )
    print(
        f"Mean CV F1:  {best_result.get('f1', 0.0):.3f} ± {best_result.get('f1_std', 0.0):.3f}"
    )
    print(f"Features: xRAG={xrag_features}, query={query_features}")

    save_obj = _fit_probe_full_data(
        X=X,
        y=y,
    ...
    metadata = {
        ...
        "cv_pr_auc_std": float(best_result.get("pr_auc_std", 0.0)),
        "cv_f1_mean": float(best_result.get("f1", 0.0)),
        "cv_f1_std": float(best_result.get("f1_std", 0.0)),
        "cv_accuracy_mean": float(best_result.get("accuracy", 0.0)),
        "cv_accuracy_std": float(best_result.get("accuracy_std", 0.0)),
    }
